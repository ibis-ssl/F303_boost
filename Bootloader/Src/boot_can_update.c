...
static void can_init(void){RCC->AHBENR|=RCC_AHBENR_GPIOAEN;RCC->APB1ENR|=RCC_APB1ENR_CANEN;GPIOA->MODER=(GPIOA->MODER&~((UINT32_C(3)<<22U)|(UINT32_C(3)<<24U)))|(UINT32_C(2)<<22U)|(UINT32_C(2)<<24U);GPIOA->AFR[1]=(GPIOA->AFR[1]&~((UINT32_C(0xF)<<12U)|(UINT32_C(0xF)<<16U)))|(UINT32_C(9)<<12U)|(UINT32_C(9)<<16U);CAN->MCR=CAN_MCR_INRQ|CAN_MCR_ABOM;while((CAN->MSR&CAN_MSR_INAK)==0U){}CAN->BTR=(UINT32_C(4)<<CAN_BTR_TS1_Pos)|(UINT32_C(1)<<CAN_BTR_TS2_Pos);CAN->FMR|=CAN_FMR_FINIT;CAN->FA1R=0U;CAN->FS1R=7U;CAN->FM1R=UINT32_C(1)<<2U;CAN->FFA1R=0U;CAN->sFilterRegister[0].FR1=COMMAND_ID<<21U;CAN->sFilterRegister[0].FR2=UINT32_C(0x7FE)<<21U;CAN->sFilterRegister[1].FR1=DATA_BASE<<21U;CAN->sFilterRegister[1].FR2=UINT32_C(0x780)<<21U;CAN->sFilterRegister[2].FR1=UINT32_C(0x010)<<21U;CAN->sFilterRegister[2].FR2=UINT32_C(0x110)<<21U;CAN->FA1R=7U;CAN->FMR&=~CAN_FMR_FINIT;CAN->MCR&=~CAN_MCR_INRQ;while((CAN->MSR&CAN_MSR_INAK)!=0U){}}
...
static void send_id(uint32_t id,const uint8_t d[8]){while((CAN->TSR&CAN_TSR_TME0)==0U){}CAN_TxMailBox_TypeDef *m=&CAN->sTxMailBox[0];m->TDTR=8U;m->TDLR=(uint32_t)d[0]|((uint32_t)d[1]<<8U)|((uint32_t)d[2]<<16U)|((uint32_t)d[3]<<24U);m->TDHR=(uint32_t)d[4]|((uint32_t)d[5]<<8U)|((uint32_t)d[6]<<16U)|((uint32_t)d[7]<<24U);m->TIR=(id<<21U)|CAN_TI0R_TXRQ;}
static void send(const uint8_t d[8]){send_id(RESPONSE_BASE+node_id,d);}
static void send_power_status(void){const uint8_t d[8]={0U};send_id(UINT32_C(0x244),d);}
...
bool boot_can_update_run(unsigned int idle_loops){node_id=board_update_node_id();can_init();for(unsigned int idle=0;idle<idle_loops||!boot_app_is_valid();idle++){uint32_t id;uint8_t d[8];IWDG->KR=UINT32_C(0xAAAA);drain();if(pop(&id,d)){idle=0U;if((id==UINT32_C(0x010)||id==UINT32_C(0x110))){send_power_status();}else if(id==UINT32_C(0x611)&&d[0]==node_id){send_version();}else if(id==COMMAND_ID){if(overflow&&receiving&&d[0]!=CMD_HELLO&&d[0]!=CMD_BEGIN){token=d[2];overflow=false;receiving=false;respond(d[0],ERR_SEQUENCE,received);}else{if(overflow){overflow=false;receiving=false;}handle(d);}}else if(id>=DATA_BASE&&id<=DATA_LAST&&receiving&&d[0]==token){const uint32_t seq=id-DATA_BASE,pos=seq*7U;if(pos<block_length&&(bitmap[seq/32U]&(UINT32_C(1)<<(seq%32U)))==0U){for(uint32_t i=1U;i<8U&&pos+i-1U<block_length;i++)block[pos+i-1U]=d[i];bitmap[seq/32U]|=UINT32_C(1)<<(seq%32U);}}}}return boot_app_is_valid();}
